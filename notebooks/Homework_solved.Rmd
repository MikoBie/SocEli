...
```{r loadLibraries}
library(tidyverse) ## every time we open RStudio we should load libraries we are going to use
```
...
```{r vaccination}
set.seed(42) ## this is important because below we use a pseudorandomization function -- sample(). It draws in random order 100 times from a vector c(TRUE, FALSE, NA) with replacement. We want to make sure that for all of us it will draw in exactly the same order. Therefore, we tell R that it should use a build-in Random Number Generation in the specific state (the number we put as an argument in the function does not matter while we all have the same). Try commenting out this function and running the code a few times. What is the difference?
